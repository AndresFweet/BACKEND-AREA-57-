...
export const gwtNewsRecents = async (req, res) => {
  try {
    ...
    const resultsFound = await pool.query(
      `SELECT * FROM work.cfg_news ORDER BY date_create DESC LIMIT 3;`
    );
...
export const getNewRequest = async (req, res) => {
  ...
  try {
    // Consulta para obtener las noticias
    const resultsFound = await pool.query(
      `SELECT * FROM work.cfg_news WHERE id = $1;`,
      [idNew]
    );
    ...
    const newsWithMedia = await Promise.all(
      resultsFound.rows.map(async (news) => {
        ...
        try {
          ...
          console.error(
            `Error al leer la carpeta de medios para el ID ${id}:`,
            err
          );
        }
...
export const getTotalNewRequest = async (req, res) => {
  try {
    ...
    const resultsFound = await pool.query(
      `SELECT * FROM work.cfg_news WHERE date_create::date = $1`,
      [today]
    );
...
export const getNewsDate = async (req, res) => {
  try {
    const date = req.params.date;

    // Consultar noticias en la base de datos
    const resultsFound = await pool.query(
      `SELECT * FROM work.cfg_news WHERE date_create::date = $1`,
      [date]
    );
    ...
    // Responder con las noticias que incluyen la URL de la imagen
    return res.json(newsWithMedia);
  } catch (error) {
    console.error("Error al obtener las noticias:", error);
